...
import { faPencil, faPlus, faTrashAlt, faFileDownload, faSpinner, faCalendarCheck, faCalendars, faCalendarXmark, faCheckToSlot, faLock } from '@fortawesome/pro-regular-svg-icons';
...
import { LichKhamFormModalComponent } from './lich-kham-form-modal/lich-kham-form-modal.component';
...
import { LapPhieuKhamBenhComponent } from './lap-phieu-kham-benh/lap-phieu-kham-benh.component';
...
export class LichKhamComponent implements OnInit {
  ...
  confirmLichKham(lichKham: any){
    const updateLichKham = this.lichKhamService.update(lichKham.id, {
      ...lichKham,
      trangThai: 'Xác nhận'
    });

    const confirmLichKham = this.lichKhamService.confirmLichKham(lichKham);

    forkJoin([updateLichKham, confirmLichKham]).subscribe((data: any) => {
  ...
  openFormModal(lichKham?: any) {
    const ref = this.dialog.open(LichKhamFormModalComponent, { size: 'lg' });
  ...
  openComfirmModal(user: any) {
    if(this.role === UserRoleEnum.ADMIN){
      ...
      ref.componentInstance.modalParams = { title: 'Xóa lịch khám', content: 'Bạn có đồng ý xóa lịch khám này?' };
      ref.result.then(deleteUser => {
          if (deleteUser) {
              this.lichKhamService.remove(user.id).subscribe(res => {
  ...
  printPhieuKham(lichKham: any){
    const ref = this.dialog.open(LapPhieuKhamBenhComponent, { size: 'lg' });
    ref.componentInstance.modalParams = { lichKham: lichKham };
    ref.result.then(dataChange => {
      if (dataChange) {
        this.loadLichKhamList();
      }
    }, () => { })
