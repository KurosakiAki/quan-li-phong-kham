...
export class LapHoaDonComponent implements OnInit {
  ...
  getThuocList() {
    this.phieuDichVuService.listThuoc().pipe(takeUntil(this.destroy$)).subscribe((list: any) => {
  ...
  removeChiTietDichVu(i: number) {
    ...
    this.donViDichVu[i] = 'Lần';
    this.thanhTienDichVu[i] = 0;
    this.donGiaDichVu[i] = 0;
    this.sum();
  }
  ...
  onSubmit(print?: any){
    ...
    if(this.hoaDonFormModal.valid && this.checkTonKho){
      this.hoaDonService.create({
        khachHangId: this.phieuDichVu.lichKham?.khachHangId,
        ...
        observables.push(this.phieuDichVuService.update(this.phieuDichVu.id, {
          ...this.phieuDichVu,
